...
def generuj_pdf(df_sorted, nazwa_zawodow=""):
    buffer = io.BytesIO()
...
def wyswietl_pdf(pdf_bytes):
    pdf_bytes.seek(0)
    b64 = base64.b64encode(pdf_bytes.read()).decode()
    pdf_display = f'<iframe src="data:application/pdf;base64,{b64}" width="100%" height="600"></iframe>'
    st.markdown(pdf_display, unsafe_allow_html=True)
    st.info("📌 Na urządzeniach mobilnych użyj funkcji 'zapisz jako PDF' w przeglądarce.")
...
if S["etap"] == 1:
    ...
    # Informacja o liczbie zawodników na sektor
    if S["liczba_zawodnikow"]>0 and S["liczba_sektorow"]>0:
        ...
        if remainder!=0:
            st.warning("⚠️ Jeden sektor może mieć o 1 zawodnika więcej.")
    ...
    for i in range(S["liczba_sektorow"]):
        ...
        if pola.strip():
            try:
                ...
                st.warning(f"Błędne dane w sektorze {nazwa}")
    ...
    with col2:
        if st.button("➡️ Przejdź do wprowadzenia wyników"):
            if len(S["zawodnicy"])==0:
                st.warning("Najpierw dodaj zawodników")
            else:
                S["etap"]=4
                zapisz_dane(S)
    ...
    if not S["zawodnicy"]:
        ...
        pdf_bytes = generuj_pdf(df_sorted,S.get("nazwa_zawodow",""))
        wyswietl_pdf(pdf_bytes)
